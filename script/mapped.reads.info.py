    ...
    with open(bed_file, "r") as bed:
        for line in bed:
            ...
            for read in bam.fetch(chr, start, end):
                ...
                read_name = read.query_name  # read的ID
                ...
                # 如果字典中已有该read_id，进行比较并保留mapping_quality较高的
...
def main(args=None):
    ...
    ref_output_file = f'{prefix}_ref_sv_reads_info.tsv'  # 指定输出文件的路径和名称
    df_ref.to_csv(ref_output_file, sep='\t', index=False)  # 保存时不包括索引
    ...
    alt_output_file = f'{prefix}_ref_alt_sv_reads_info.tsv'  # 指定输出文件的路径和名称
    df_alt.to_csv(alt_output_file, sep='\t', index=False)  # 保存时不包括索引
